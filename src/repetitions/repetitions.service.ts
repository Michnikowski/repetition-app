...
export class RepetitionsService {
  async getTodayUserWord(user: User) {
    ...
    const repetitionWord = await Word.createQueryBuilder('word')
      ...
      .andWhere('userWord.repetitionDate < :repetitionDate', {
        repetitionDate: tomorrow,
      })
      .andWhere('userWord.wordStatus = :wordStatus', {
    ...
    const wordRepetitionTime: number =
      repetitionWord.userWords[0].repetitionTime;
  ...
  async updateUserWord(user: User, wordDetails: UpdateUserWordDto) {
    const word = await Word.findOne(wordDetails.wordId);

    const lastUpdatedDate: Date = new Date();

    const userWord = await UserWord.createQueryBuilder('userWord')
      .where('userWord.wordId = :wordId', { wordId: `${wordDetails.wordId}` })
      .andWhere('userWord.userId = :userId', { userId: `${user.id}` })
      .getOne();

    const log = new Log();
    log.actionDate = lastUpdatedDate;
    log.repetitionTime = RepetitionTime[wordDetails.repetitionTime];
    log.user = user;
    log.word = word;

    if (RepetitionTime[wordDetails.repetitionTime] > userWord.repetitionTime) {
      log.actionType = ActionType.LEVEL_UP;
    } else if (
      RepetitionTime[wordDetails.repetitionTime] < userWord.repetitionTime
    ) {
    ...
    const newRepetitionDate: Date = new Date(
      format(
        addDays(new Date(), RepetitionTime[wordDetails.repetitionTime]),
        'yyyy-MM-dd',
      ),
    );

    userWordToUpdate.lastUpdatedDate = lastUpdatedDate;
    userWordToUpdate.repetitionTime =
      RepetitionTime[wordDetails.repetitionTime];
    userWordToUpdate.repetitionDate = newRepetitionDate;

    if (
      RepetitionTime[wordDetails.repetitionTime] === 60 &&
      userWord.repetitionTime === 60
    ) {
